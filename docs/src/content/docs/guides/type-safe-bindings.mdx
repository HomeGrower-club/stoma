...
When using [Service Binding upstreams](/concepts/upstreams/), the `service` field is a string that must match a binding name in your `wrangler.toml`. A typo means a runtime 502 error. The `TBindings` generic parameter lets TypeScript catch these mistakes at compile time and gives you IDE autocomplete for valid binding names.
...
Define an `Env` interface that matches your `wrangler.toml` bindings, then pass it to `createGateway`:

```typescript
import { createGateway, jwtAuth } from "@homegrower-club/stoma";

// Your Worker's Env interface (matches wrangler.toml bindings)
interface Env {
