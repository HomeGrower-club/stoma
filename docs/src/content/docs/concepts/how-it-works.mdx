...
1. Extracts the client IP (from `x-forwarded-for` or `cf-connecting-ip` if you're on Cloudflare)
...
The upstream responds. Now the response travels back through the policies in **reverse order** (this is how Hono middleware works). Policies that ran before can now modify the response.
...
Every policy is a Hono middleware. If you know how to write Hono middleware, you can write a Stoma policy. No special abstractions, no framework to learn.
